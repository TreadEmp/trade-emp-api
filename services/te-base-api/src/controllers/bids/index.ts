...
import { BidResponseModel } from "./apiModels/bidResponseModel";
...
export const createOrUpdateBids = async (req: Request, res: Response) => {
    try {
        const employeeId = getUserId(res);
        const bidsDto: BidsDto = plainToClass(BidsDto, req.body, {
            enableImplicitConversion: false
        });
        bidsDto.employeeId = employeeId;
        if (bidsDto.id === null || bidsDto.id === undefined) {
            bidsDto.id = Date.now().toString();
        }
        ...
        const returnResult = await bidsService.createOrUpdateBids(bidsDto);
        ...
        _logger.error(
            "An Exception occurred in createOrUpdateBids" + error
        );
